...
const BlueChartPage: React.FC = () => {
    ...
    historicalData.length > 0
      ...
      : Array.from({ length: 50 }, (_, i) => ({
          ...
          temperature: {
            ...
            blue: 18 + Math.random() * 10 + Math.cos(i / 4) * 3,
  ...
  const currentTemp =
    historicalData.length > 0
      ? currentTemperatures.blue
      : blueTemps[blueTemps.length - 1] || 0;
  const maxTemp = Math.max(...blueTemps);
  const minTemp = Math.min(...blueTemps);
  const avgTemp =
    blueTemps.reduce((sum: number, temp: number) => sum + temp, 0) /
    blueTemps.length;
      ...
      <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        >
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            <ThermostatOutlined sx={{ fontSize: 40, color: "#2196f3" }} />
            <Typography variant="h4" component="h1">
              Blue Sensor Analysis
            </Typography>
          </Box>
        ...
        <Paper elevation={3} sx={{ p: 3 }}>
          <Typography variant="h5" gutterBottom sx={{ color: "#2196f3" }}>
            Blue Sensor Temperature History
          </Typography>
        ...
        <Paper elevation={3} sx={{ p: 3 }}>
          ...
          <Box sx={{ height: "300px", width: "100%" }}>
            <LineChart
              ...
              series={[
                {
                  data: blueTemps.slice(-12), // Last 12 data points
                  label: "Blue Sensor (Recent)",
                  color: "#2196f3",
                  curve: "natural",
                },
              ]}
